...
contract MarketSwap {
    ...
    function viewPrice() pure public returns(uint price){
        return (uint256(price));
    }

    function checkToken(address _fromToken,address _toToken,uint _amountIn) public{
        if(_fromToken == DAI){
            swapDaiUsdtTokens (_fromToken, _toToken,_amountIn);
        } else if (_fromToken == USDT) {
            swapUsdtDaiTokens (_fromToken, _toToken,_amountIn);
        }
    }

    function swapDaiUsdtTokens (address _fromToken,address _toToken,uint _amountIn) internal{
    ...
    function swapUsdtDaiTokens(address _fromToken,address _toToken,uint _amountIn) internal{
